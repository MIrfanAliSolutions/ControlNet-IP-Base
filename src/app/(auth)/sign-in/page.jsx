...
const SignIn = () => {
  ...
  const handleLogin = async (values) => {
    ...
    if (values?.email === 'nintendo@droppgroup.xyz') {
      dispatch(
        toggleLogin({
          isLogin: true,
        })
          ...
          <Form className='w-full text-white text-sm md:text-base pt-2 md:pt-4 z-10 relative'>
            ...
            <div className='w-full pt-4'>
              <button
                ...
                className='btn-login bg-Gradient text-base text-black w-full leading-3 md:leading-5 py-3 md:py-4 text-center rounded-2xl cursor-pointer font-semibold flex items-center justify-center'
              >
                {isApiLoading ? (
                  <RotatingLines
                    visible={true}
